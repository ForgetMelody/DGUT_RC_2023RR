...
from selfaim.cfg import yawConfigConfig

class custompoint2Pointcloud2:
    def __init__(self) -> None:
        rospy.init_node("customp2pcl")
        ...
        # self.search_radius = rospy.get_param('~search_radius',default = 0.30)   # 匹配点云的限制距离
        # self.limit_yaw=rospy.get_param('~limit_yaw',default = 0.01)  #当我们的yaw差的值小于这个数就继承上一次的yaw
        # self.compensation=rospy.get_param('~compensation',default = 0.05)  #对于距离上的补偿
        # self.aim_distance=rospy.get_param('~aim_distance' , default = 1.8)  #点云识别方法的阈值
        # self.yaw_diff_long = rospy.get_param('~yaw_diff_long',default = 0.5)  #对于偏航的补偿
        # self.yaw_diff_long /= 180*np.pi
        # self.yaw_diff_short = rospy.get_param('~yaw_diff_short',default = 1.2)  #对于偏航的补偿
        # self.yaw_diff_short /= 180*np.pi
        # self.yaw_offsets_k = 0.5  #线性补偿的k
        # self.yaw_offsets_b = 2.0  #线性补偿的b
        # self.y_offsets = rospy.get_param('~y_offsets',default= 0.01)   #对于点云点的y轴补偿
        # self.tolerate_yaw = rospy.get_param('~tolerate_yaw',default= 0.02)   #对于偏航的tolerate

        rospy.wait_for_message("/Odometry", Odometry,timeout=None)  #Odometry的msg
        # self.pos_filtered = self.Odometry_callback(ordem_msg)
        rospy.Subscriber("/Odometry",Odometry,self.Odometry_callback)

        rospy.Subscriber("/livox/lidar",CustomMsg,self.CustomMsg2PointCloud2_callback)
        self.srv=Server(yawConfigConfig,self.param_callback)
        init=yawConfigConfig.defaults
        self.srv.update_configuration(init)
        ...
        #         self.yaw_msg.yaw_msg=self.yaw_group
        #         self.yaw_msg.dis_msg=self.dis_group
        #         self.pub_yaw.publish(self.yaw_msg)

    def param_callback(self,config,level):
        self.search_radius = config.search_radius   # 匹配点云的限制距离
        self.limit_yaw=config.limit_yaw  #当我们的yaw差的值小于这个数就继承上一次的yaw
        self.compensation=config.compensation  #对于距离上的补偿
        self.aim_distance=config.aim_distance  #点云识别方法的阈值
        self.yaw_diff_long = config.yaw_diff_long  #对于偏航的补偿
        self.yaw_diff_long /= 180*np.pi
        self.yaw_diff_short = config.yaw_diff_short  #对于偏航的补偿
        self.yaw_diff_short /= 180*np.pi
        self.y_offsets = config.y_offsets   #对于点云点的y轴补偿
        self.tolerate_yaw = config.tolerate_yaw   #对于偏航的tolerate
        self.yaw_offsets_k = config.yaw_offsets_k   #yaw线性补偿的k
        self.yaw_offsets_b = config.yaw_offsets_b   #yaw线性补偿的b
        return config
    ...
    def Odometry_callback(self,msg: Odometry):  #读取Odometry，即雷达信息
        ...
        self.pos_filtered = self.position_filter.update(pos)
    ...
    def CustomMsg2PointCloud2_callback(self,msg:CustomMsg):
        ...
        for i in custompoint:
            if 0.06<i.z<0.5:  #筛选中间层的点云
                # x = i.x + self.y_offsets*np.cos(self.pos_filtered[2])
                # y = i.y + self.y_offsets*np.sin(self.pos_filtered[2])
                x=i.x
                y=i.y
                z = 1.0
    ...
    def CustomMsg2PointCloud2_callback(self,msg : CustomMsg):  #CustomMsg转点云的callback
        ...
        # for i in self.target:
            # i[0]+=self.y_offsets*np.cos(self.pos_filtered[2]+np.pi/2)
            # i[1]+=self.y_offsets*np.sin(self.pos_filtered[2]+np.pi/2)

        for i in custompoint:
            if 0.07<i.z<0.35:  #筛选中间层的点云
                # x = i.x + self.y_offsets*np.cos(self.pos_filtered[2]+np.pi/2)
                # y = i.y + self.y_offsets*np.sin(self.pos_filtered[2]+np.pi/2)
                x=i.x
                y=i.y
                z = 1.0
        ...
        self.cloud_msg,self.cloud_points=cloud_msg,cloud_points
    ...
    def match_cloud(self):  #匹配点云
        ...
        for ttarget in self.trans_target:
            # 查询 对应柱子坐标 半径为 self.search_radiusm 的圆内的所有坐标点
            points=[]
    ...
    def caculate_yaw(self):  #计算偏航
        ...
        for i in mid_point:
            ...
            offset=0
            offset= self.yaw_offsets_k*dis_group[index]+self.yaw_offsets_b  #根据柱子的距离给一个偏航的补偿
            # rad=atan2(i[1],i[0])
